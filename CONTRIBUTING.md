...
If you want to use Android Studio or Xcode to edit the native code, you can open the `example/android` or `example/ios` directories respectively in those editors. To edit the Objective-C or Swift files, open `example/ios/BleSdkExample.xcworkspace` in Xcode and find the source files at `Pods > Development Pods > ble_sdk`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `ble-sdk` under `Android`.
...
```sh
Running "BleSdkExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
